...
def main():
    logger.info("Iniciando Proceso Por Lotes - ReplicaICMVS-Variable")
    ...
    try:
        ...
        if rows_count == 0:
            ...
            for offset in range(0, rows_count, limit):
                ...
                pd.DataFrame(df.iloc[offset:offset + limit]).to_csv(f"{root}/ReplicaICMVS-Variable-{count_archivos}.txt", index=False, sep=';')
                count_archivos += 1
        else:
            logger.info(f"Generando archivo, lote No.1")
            df = get_data_query(query=query, model=model)
            df.to_csv(f'{root}/ReplicaICMVS-Variable-1.txt', index=False, sep=';')
