    ...
    try:
        ...
        if rows_count == 0:
            ...
            for offset in range(0, rows_count, limit):
                ...
                pd.DataFrame(df.iloc[offset:offset + limit]).to_csv(f"{root}/ReplicaICMVS-Sugerida-{count_archivos}.txt", index=False, sep=';')
                count_archivos += 1
        else:
            logger.info(f"Generando archivo, lote No.1")
            df = get_data_query(query=query, model=model)
            df.to_csv(f'{root}/ReplicaICMVS-Sugerida-1.txt', index=False, sep=';')
