...
def main():
    logger.info("Iniciando Proceso Por Lotes - ReplicaICMVS-Sugerida")
    ...
    query = '''
        ...
        AND "ZUTIMER" = ( SELECT MAX("ZUTIMER")
                      FROM "ReplicaICMVS"
                      WHERE "DATUM" = (
                        SELECT MAX("DATUM")
                        FROM "ReplicaICMVS"
    ...
    try:
        ...
        if rows_count == 0:
            ...
            for offset in range(0, rows_count, limit):
                ...
                # Crear copia del fragmento para evitar problemas al modificar
                df_temp = df.iloc[offset:offset + limit].copy()

                # Formatear SOLO las columnas indicadas
                ...
                df_temp.to_csv(
                    f"{root}/ReplicaICMVS-Sugerida-{count_archivos}.txt",
                    index=False,
                    sep=';',
                    float_format="%.2f"
                )
            ...
            df_temp.to_csv(
                f'{root}/ReplicaICMVS-Sugerida-1.txt',
                index=False,
                sep=';',
                float_format="%.2f"
            )
