import os, json, time, logging, sys, shutil
...
def get_data_query(query, limit=99999999, chunk_size=100000, model='', eff=True):
...
def main():
    logger.info("Iniciando proceso de generacion y reubicacion de archivos pgp.")

    # Ruta inicial de los archivos
    root = os.path.join("root", "Publication")
    # Ruta destino
    final_root = os.path.join("root", "Publication", "Azure", "Salida")

    # Archivos a identificar
    files = [
        "XXICMSALDOS2",
        "XXICMCONSOLIDADOCOMISIONES",
        "XXICMCOMISIONES"
    ]

    for file in files:
        logger.info(f"----> Buscando archivo {file} mas reciente. <----")

        # Se sacan todos los archivos que tengan similitud con el nombre del archivo XX buscado
        filesSFTP = [
            os.path.join(root, f)
            for f in os.listdir(root)
            if f.endswith(".txt") and file in f
        ]

        if len(filesSFTP) == 0:
            logger.info(f"Ningun archivo {file} fue encontrado en las publicaciones.")
        else:
            # Se saca el nombre del archivo con la fecha de creacion mas reciente
            archivo_mas_reciente = (max(filesSFTP, key=os.path.getmtime)).split('/')[-1]
            logger.info(f"Procesando archivo {file} mas reciente: {archivo_mas_reciente} .")

            # Limpieza de archivos viejos
            if len(final_root) >= 2:
                for root_old_file in filesSFTP:
                    old_file = root_old_file.split('/')[-1]
                    if old_file != archivo_mas_reciente and os.path.exists(os.path.join(root, old_file)):
                        os.remove(os.path.join(root, old_file))

            # Verificamos una vez mas por seguridad que el archivo mas reciente exista
            if os.path.exists(os.path.join(root, archivo_mas_reciente)):

                # Copiamos el archivo mas reciente a la ruta final y lo pegamos con el nombre plano
                # basandonos en file. Aplicando tambien el cambio de extencion a csv
                shutil.copy2(os.path.join(root, archivo_mas_reciente), os.path.join(final_root, f"{file}.csv"))
                if os.path.exists(os.path.join(final_root, f"{file}.csv.pgp")):
                    os.remove(os.path.join(final_root, f"{file}.csv.pgp"))

                if not os.path.exists(os.path.join(final_root, "OK.txt")):
                    with open(os.path.join(final_root, "OK.txt"), 'w') as archivo:
                        pass  # No hacemos nada, solo lo abrimos y cerramos

            else:
                logger.info(f"No existe {archivo_mas_reciente}")
