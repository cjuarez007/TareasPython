...
def main():
    logger.info("Iniciando proceso de borrado - Archivos incentivos categoria - SFTP DEV")

    # Ruta de trabajo
    root = os.path.join("root", "Data")
    # Archivos para borrar de IncentivoCategoria
    files_categoria = [
        "Categoria-VentaUnidades.csv",
        "Categoria-VentaMonto.csv",
        "Categoria-MontoDistribuible.csv",
        "Categoria-EmpleadosActivos.csv"
    ]

    try:
        for file in files_categoria:
            logger.info(f"Borrando archivo: {file} .")
            root_file = os.path.join(root, file)

            if os.path.exists(root_file):
                os.remove(root_file)
                logger.info(f"Archivo {file} eliminado.")
            else:
                logger.info(f"El archivo {file} intento ser borrado pero no existe.")

    except Exception as ex:
        logger.error(f"La eliminacion del archivo {file} fallo. Detalles: {ex}")
